		...
		while (enet_host_service(enet_handle, &event, 0) > 0) {
			switch (event.type) {
				case ENET_EVENT_TYPE_CONNECT:
					printf("error: received a connection event from %s:%s", ipToString(event.peer->address.host).c_str(),std::to_string(event.peer->address.port).c_str());
					break;
				case ENET_EVENT_TYPE_RECEIVE:
				...
				case ENET_EVENT_TYPE_DISCONNECT:
					printf("%s disconnected.", (char *) event.peer->data);
					/* Reset the peer's client information. */
					event.peer->data = NULL;
			}
		}
